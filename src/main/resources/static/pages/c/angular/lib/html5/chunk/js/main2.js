...
$(function () {
    ...
    var url = '/group/uploadChunkResource.action';
    var __uploadedBytes = 0, __maxChunkSize = 1000000; //1M
    $('#fileupload').unbind().fileupload({
        ...
        add: function (e, data) {
            ...
            $('.tips').hide();
            _element = '';
            parent.moveToRightConner();//添加文件上传后，窗口从中间移动到右下角，并开始上传
    ...
    $('#files').bind('click', function (e) {
        ...
        if (target.is('.onecancel')) {
            ...
            if ($('#resourceId').val() > 0) {
                $.ajax({
                    url: '/group/deleteChunckResource.action',
                    dataType: 'json',
                    type: 'post',
...
function getChunk() {
    return $.ajax({
        url: '/group/getMyChunckResources.action',
        dataType: 'json',
        type: 'post'
    })
}
