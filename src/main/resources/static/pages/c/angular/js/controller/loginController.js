angular.module("LoginModule", ["ngCookies", "httpRequest"])
  .controller("LoginController", ["$scope", "$cookies", "global.staticInfo", "httpRequest.sendRequest", "$state",
    "$timeout", "$rootScope", function ($scope, $cookies, staticInfo, sendRequest, $state, $timeout, $rootScope) {
      ...
      $scope.$watch("casOptions", function (newVal, oldVal) {
        if (newVal === '本地认证') {
          ...
          angular.element(document.querySelector('form[name="casForm"]')).attr('action',
            Config.casPath + 'login?service=' +
            encodeURIComponent(Config.sitePath) + '%2Fpages%2FcasLogin.jsp');//设置action
        }
      });
