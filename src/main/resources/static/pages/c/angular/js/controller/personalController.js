...
PersonalController.controller('PersonalController', ['$scope', '$http', '$rootScope', '$state', 'httpRequest.sendRequest', 'modalWindow', 'modalConfirm', '$timeout',
  function ($scope, $http, $rootScope, $state, sendRequest, modalWindow, modalConfirm, $timeout) {
    $scope.id = 0;
    $scope.isHidden = true;
    ...
    $scope.gridOptions = {
      ...
      init: function () {
        // angular.element(document.querySelector(".ngViewport")).height(angular.element(window).height() - 120);
        // angular.element(window).resize(function () {
        //   angular.element(document.querySelector(".ngViewport")).height(angular.element(window).height() - 125);
        // });
      },
    ...
    //resumeUpload 断点上传
    $rootScope.resumeUpload = function () {
      $('.am-dropdown').dropdown('close');//关闭下拉
      if (!$scope.uploadShow) {
        if ($scope.isuploading) {
          modal.show();
          $scope.uploadShow = true;
          ...
          modal = modalWindow.open({
            template: '<iframe src="lib/html5/chunk/upload.html#parentId=' + $scope.listOptions.parentId + '&groupId=' + $rootScope.status.personGroupId + '&singleSize=2097152&leaveSpace=20953448" frameborder="0" scrolling="no" style="width:100%;height:450px;"></iframe>',
            title: '断点上传',
            width: 334,
            height: 121,
            onCloseClicked: function () {
    ...
    sendRequest('/user/status.action', {}, function (data) {
      if (data.status == 'login') {
        ...
        $scope.id = -$rootScope.status.personGroupId;
        // if (localStorage.personalId !== undefined) {
        //   $scope.id = -localStorage.personalId;
        // } else {
        //   $scope.id = -$rootScope.status.personGroupId;
        //   localStorage.personalId = $rootScope.status.personGroupId;
        // }
        $scope.getPagedDataAsync($scope.pagingOptions.pageSize, ($scope.pagingOptions.currentPage - 1) * $scope.pagingOptions.pageSize, $scope.id);
      } else {
...
function moveToRightConner() {
  $('.am-modal').addClass('moveToRightConner');
}
...
var GridLayout = {
  ...
  resize: function ($scope) {
    setTimeout(function () {
      ...
      var __scope = angular.element(document.querySelector('.ngGrid')).scope();
      __scope.gridOptions.$gridServices.DomUtilityService.RebuildGrid(__scope.gridOptions.$gridScope,
        __scope.gridOptions.ngGrid);
    }, 200);
