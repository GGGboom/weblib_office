...
function ngGridDraggableRow(initialSortField, Request,$rootScope) {
    ...
    var modalConfirm = initialSortField;
    ...
    function Move(rowItemsMove, rowItemDst) {
        if (rowItemDst.entity.type == 1 && rowItemsMove.length > 0) {
            //$rootScope.mcId = [];
            MoveId = [];
            groupId = rowItemDst.entity.groupId;
            parentId = rowItemDst.entity.id;
            for (var i = 0; i < rowItemsMove.length; i++) {
                MoveId.push(rowItemsMove[i].entity.id)
            }
            dragModal = modalConfirm.confirm({
                title:  '是否将这' + rowItemsMove.length + '个资源移动到: ' + rowItemDst.entity.displayName+" 文件夹中 ？",
                onConfirm: function (data) {
                    $rootScope.progressbar.start();
                    sendRequest('/group/moveResource.action',
                        {
                            groupId: groupId,
                            parentId: parentId,
                            id: MoveId
...
//module.exports = ngGridDraggableRow;