...
public class Crypt {
	...
	//私钥解密
	public static String RsaDecrypt(String data, RSAPrivateKey privateKey) throws Exception{
		...
		byte[] bcd = Base64.decode(data);
		//System.err.println(bcd.length);
		//如果密文长度大于模长则要分组解密
		String decrypted = "";
		byte[][] arrays = splitArray(bcd, key_len);
		for(byte[] arr : arrays){
			decrypted += new String(cipher.doFinal(arr));
		}
		return decrypted;
